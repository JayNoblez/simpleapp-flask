...
if [ -f "/tmp/tools/yq" ]; then
    ...
    chmod +x /tmp/tools/yq && sudo cp /tmp/tools/yq /usr/bin/yq
else
    echo "Installing yq"
...
if [ -f "/tmp/tools/helm" ]; then
    ...
    chmod +x /tmp/tools/helm && sudo cp /tmp/tools/helm /usr/local/bin/helm
else
    echo "Installing helm"
    curl -LO https://get.helm.sh/helm-v3.7.2-linux-arm64.tar.gz
    tar -zxvf /tmp/tools/helm-v3.7.2-linux-arm64.tar.gz
    sudo cp /tmp/tools/linux-arm64/helm /tmp/tools/helm
    sudo cp /tmp/tools/helm /usr/local/bin/helm
fi